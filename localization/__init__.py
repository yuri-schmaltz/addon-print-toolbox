...
def _init() -> dict[str, dict[tuple[str, str], str]]:
    # Keep initialization side-effect free: parsing .po files is cheap for this
    # addon and avoids write failures in read-only install directories.
    return {locale: trnsl for locale, trnsl in _walk()}
